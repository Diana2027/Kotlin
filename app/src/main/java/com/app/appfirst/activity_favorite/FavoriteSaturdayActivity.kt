...
import com.app.appfirst.activity_detail.DetailSaturdayActivity
import com.app.appfirst.adapter_days.SaturdayAdapter
import com.app.appfirst.all_data.DataSaturday

class FavoriteSaturdayActivity : AppCompatActivity() {

    companion object {
        val key = "favoriteSaturdayKey"
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_saturday_favorite)
        supportActionBar?.title = "Избранное"
        intent.getParcelableArrayListExtra<DataSaturday>(key)?.let { initList(it) }
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.setDisplayShowHomeEnabled(true)
    }

    // инициализация списка. Если список избранных пустой, отобразить текст
    private fun initList(list: ArrayList<DataSaturday>) {
        ...
        if (favoriteList.isEmpty()) {
            ...
            recyclerView.adapter = SaturdayAdapter(favoriteList as ArrayList<DataSaturday>, {
                navigateToDetail(it)
            }, {}, false)
        }
    }

    private fun navigateToDetail(item: DataSaturday) {
        Intent(this, DetailSaturdayActivity::class.java).also {
            it.putExtra(DetailSaturdayActivity.key, item)
            startActivity(it)
        }
    }
