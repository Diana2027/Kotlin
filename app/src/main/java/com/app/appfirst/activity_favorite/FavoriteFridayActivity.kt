...
class FavoriteFridayActivity : AppCompatActivity() {

    companion object {
        val key = "favoriteFridayKey"
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_friday_favorite)
        supportActionBar?.title = "Избранное"
        intent.getParcelableArrayListExtra<DataFriday>(key)?.let { initList(it) }
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        supportActionBar?.setDisplayShowHomeEnabled(true)
    }

    // инициализация списка. Если список избранных пустой, отобразить текст
    private fun initList(list: ArrayList<DataFriday>) {
        ...
        if (favoriteList.isEmpty()) {
            ...
            recyclerView.adapter = FridayAdapter(favoriteList as ArrayList<DataFriday>, {
                navigateToDetail(it)
            }, {}, false)
        }
    }
